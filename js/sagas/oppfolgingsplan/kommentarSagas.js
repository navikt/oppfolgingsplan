...
export function* lagreKommentar(action) {
  ...
  try {
    ...
    )}/tiltak/actions/${action.tiltakId}/lagreKommentar`;
...
export function* slettKommentar(action) {
  ...
  try {
    const url = `${hentSyfoapiUrl(
      API_NAVN.SYFOOPPFOLGINGSPLANSERVICE
    )}/kommentar/actions/${action.kommentarId}/slett`;
    yield call(post, url);
    yield put(
    ...
    log(e);
    yield put(
      actions.slettKommentarFeilet(fnr, action.tiltakId, action.kommentarId)
    );
  }
}
