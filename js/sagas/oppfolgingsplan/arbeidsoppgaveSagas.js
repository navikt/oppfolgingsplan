...
export function* lagreArbeidsoppgave(action) {
  ...
  yield put(
    actions.lagrerArbeidsoppgave(fnr, action.arbeidsoppgave.arbeidsoppgaveId)
  );
  const body = input2RSArbeidsoppgave(action.arbeidsoppgave);
  try {
    const url = `${hentSyfoapiUrl(
      API_NAVN.SYFOOPPFOLGINGSPLANSERVICE
    )}/oppfolgingsplan/actions/${action.id}/lagreArbeidsoppgave`;
    const data = yield call(post, url, body);
    yield put(
      actions.arbeidsoppgaveLagret(action.id, data, action.arbeidsoppgave, fnr)
    );
  } catch (e) {
    if (e.message === '409') {
...
export function* slettArbeidsoppgave(action) {
  ...
  try {
    const url = `${hentSyfoapiUrl(
      API_NAVN.SYFOOPPFOLGINGSPLANSERVICE
    )}/arbeidsoppgave/actions/${action.arbeidsoppgaveId}/slett`;
    yield call(post, url);
    yield put(
      actions.arbeidsoppgaveSlettet(action.id, action.arbeidsoppgaveId, fnr)
    );
  } catch (e) {
    if (e.message === '409') {
      window.location.reload();
