      ...
      >
        {(() => {
          if (arbeidsgivere.length === 1 && !arbeidsgivere[0].harNaermesteLeder) {
            return <Feilmelding tittel={texts.errorNoLeader.title} melding={texts.errorNoLeader.message} />;
          } else if (this.state.side === 0) {
            return <Feilmelding tittel={texts.errorNoGodkjentPlan.title} melding={texts.errorNoGodkjentPlan.message} />;
          }
          return (
            <div>
              {this.state.side === 1 && (
