...
export class Container extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      oppfolgingsplanHarBlittOpprettet(
        oppfolgingsdialogerReducer,
        nextProps.oppfolgingsdialogerReducer
      )
    ) {
      window.sessionStorage.setItem('hash', 'arbeidsoppgaver');
      this.props.hentOppfolgingsdialoger();
    ...
    if (
      ...
      history.push(
        `${getContextRoot()}/oppfolgingsplaner/${
          nextProps.oppfolgingsdialogerReducer.opprettetId
        }`
      );
    }
    if (
      ...
      history.push(
        `${getContextRoot()}/oppfolgingsplaner/${
          nextProps.kopierDialogReducer.data
        }`
      );
    }
  }
  ...
  render() {
    ...
    return (
      <Side
        ...
        laster={
          (henter || sender || !hentet) && !(sendingFeilet || hentingFeilet)
        }
      >
        {(() => {
          if (henter || sender) {
...
Container.propTypes = {
  ...
  oppfolgingsdialoger: PropTypes.arrayOf(
    oppfolgingsplanProptypes.oppfolgingsplanPt
  ),
...
export const mapStateToProps = (state) => {
  const oppfolgingsdialoger = state.oppfolgingsdialoger.data.map(
    (oppfolgingsdialog) => {
  ...
  return {
    ...
    sender:
      state.oppfolgingsdialoger.oppretter || state.kopierDialogReducer.sender,
    sendingFeilet:
      state.oppfolgingsdialoger.hentingFeilet ||
