...
const texts = {
  ...
  tiltakInformasjon: {
    ...
    tiltakVarselVurdering:
      'Dette tiltaket mangler en vurdering fra lederen din',
  },
};
  ...
  return (
    <div className="tiltaktabell__rad__utvidbar--rad">
      {tiltak && tiltak.beskrivelse && (
        <label className="tiltaktabell--beskrivelse">
          {texts.tiltakInformasjonBeskrivelse.label}
        </label>
      )}
  ...
  return (
    <div className="tiltaktabell__rad__utvidbar--rad">
      {tiltak && tiltak.gjennomfoering && (
        <label className="tiltaktabell--oppfoelging">
          {texts.tiltakInformasjonGjennomfoering.label}
        </label>
      )}
  ...
  return (
    <div className="tiltaktabell__rad__utvidbar--rad">
      {tiltak && tiltak.beskrivelseIkkeAktuelt && (
        <label className="tiltaktabell--oppfoelging">
          {texts.tabellTiltakBeskrivelseIkkeAktuelt.label}
        </label>
      )}
      {tiltak && tiltak.beskrivelseIkkeAktuelt && (
...
class TiltakInformasjon extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      this.props.element.tiltakId ===
        nextProps.kommentarReducer.feiletTiltakId &&
      nextProps.kommentarReducer.lagringFeilet &&
      nextProps.kommentarReducer.lagringFeilet !==
        this.props.kommentarReducer.lagringFeilet
    ) {
  ...
  render() {
    ...
    return (
      <div className={'tiltaktabell__rad__utvidbar tiltakInformasjon'}>
        ...
        {visVarsel(fnr, element) && (
          <TiltakVarselVurdering
            tekst={texts.tiltakInformasjon.tiltakVarselVurdering}
          />
        )}

        {element &&
          element.gjennomfoering &&
          element.status === STATUS_TILTAK.AVTALT && (
            <TiltakInformasjonGjennomfoering tiltak={element} />
          )}

        {element &&
          element.beskrivelseIkkeAktuelt &&
          element.status === STATUS_TILTAK.IKKE_AKTUELT && (
        ...
        {lagreKommentarSkjema && (
          <LagreKommentarSkjema
            ...
            kommentarFeilet={
              this.state.visLagringKommentarFeilet && feilMelding
            }
