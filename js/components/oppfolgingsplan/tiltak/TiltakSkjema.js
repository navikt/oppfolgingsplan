...
export const TiltakBeskrivelse = ({
  ...
  return tiltak &&
    tiltak.opprettetAv &&
    !aktoerHarOpprettetElement(fnr, tiltak) ? (
    <div className="lagretiltakskjema__inputgruppe">
      {tiltak &&
        tiltak.beskrivelse && [
          <label
            key={`tiltak-besk-label-${tiltak.tiltakId}`}
            className="tiltaktabell--beskrivelse"
          >
...
export class TiltakSkjemaKomponent extends Component {
  ...
  handleInitialize() {
    const tiltak = this.visFeiletTiltak()
      ? this.props.tiltakReducer.tiltak
      : this.props.tiltak;
    const initData = {};
    if (tiltak) {
      ...
      initData.opprettetAv = this.props.tiltak
        ? this.props.tiltak.opprettetAv
        : null;
    } else {
  ...
  handleSubmit(values) {
    ...
    if (feilmeldingerObject.tiltaknavn) {
      ...
      errorList.push({
        skjemaelementId: FELTER.tiltaknavn.id,
        feilmelding: feilmeldingerObject.tiltaknavn,
      });
    }
    ...
    if (feilmeldingerObject.fom) {
      ...
      errorList.push({
        skjemaelementId: FELTER.startdato.id,
        feilmelding: feilmeldingerObject.fom,
      });
    }
    ...
    if (feilmeldingerObject.tom) {
      ...
      errorList.push({
        skjemaelementId: FELTER.sluttdato.id,
        feilmelding: feilmeldingerObject.tom,
      });
    }
  ...
  validateSluttDato = (value) => {
    ...
    if (
      ...
      value &&
      !sluttDatoSenereEnnStartDato(
        konvertDatoTiltakMedPunkt(this.state.fom),
        konvertDatoTiltakMedPunkt(value)
      )
    ) {
