...
export class Container extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    const {
      ...
      avbrytdialogReducer,
      dialogAvbruttOgNyOpprettetConnected,
    } = this.props;
    if (
      oppfolgingsplanHarBlittAvbrutt(
        avbrytdialogReducer,
        nextProps.avbrytdialogReducer
      )
    ) {
      this.props.hentOppfolgingsdialoger();
    }
    if (
      ...
      oppfolgingsdialogerReducer.henter &&
      ...
      if (nyOpprettetDialog) {
        ...
        history.push(
          `${getContextRoot()}/oppfolgingsplaner/${nyOpprettetDialog.id}`
        );
...
export function mapStateToProps(state, ownProps) {
  ...
  let oppfolgingsdialog = getOppfolgingsdialog(
    state.oppfolgingsdialoger.data,
    id
  );
  oppfolgingsdialog =
    oppfolgingsdialog && populerPlanFraState(oppfolgingsdialog, state);
