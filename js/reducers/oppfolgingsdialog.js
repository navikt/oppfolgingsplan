...
import { finnNyesteGodkjenning, skalDeleMedNav } from '../utils/oppfolgingsdialogUtils';
...
const oppfolgingsdialoger = (state = initiellState, action = {}) => {
    switch (action.type) {
        ...
        case actions.DIALOG_GODKJENT: {
            const data = state.data.map((oppfolgingsdialog) => {
                if (Number(action.id) === oppfolgingsdialog.id) {
                    if (oppfolgingsdialog.godkjenninger.length > 0 && finnNyesteGodkjenning(oppfolgingsdialog.godkjenninger).godkjent) {
                        return Object.assign({}, oppfolgingsdialog, {
                            godkjenninger: [],
                            status: 'AKTIV',
                            godkjentPlan: {
        ...
        case actions.DIALOG_AVVIST: {
            const data = state.data.map((oppfolgingsdialog) => {
                if (Number(action.id) === oppfolgingsdialog.id) {
                    return Object.assign({}, oppfolgingsdialog, {
                        godkjenninger: [{ godkjent: false, godkjenningsTidspunkt: new Date(), godkjentAv: oppfolgingsdialog.arbeidstaker }],
                        sistEndretAv: oppfolgingsdialog.arbeidstaker,
                        sistEndretDato: new Date(),
