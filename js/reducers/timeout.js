import { FORLENG_INNLOGGET_SESJON, SJEKK_INNLOGGINGSSESJON, SNART_UTLOGGET } from '../timeout/timeout_actions';

const STANDARD_TIMEOUT = 30;
const VIS_TIMEOUTBOKS_MINUTTER_FOER_UTLOGGING = 5;
const MILLIES_MELLOM_VIS_BOKS_OG_TIMEOUT = VIS_TIMEOUTBOKS_MINUTTER_FOER_UTLOGGING * 60 * 1000;

const hentErInnloggetTil = function hentErInnloggetTil() {
  const date = new Date();
  date.setMinutes(date.getMinutes() + STANDARD_TIMEOUT);
  return date;
};

const erBrukerUtlogget = function erBrukerUtlogget(state) {
  return state.erInnloggetTil.getTime() - new Date().getTime() < 0;
};

const erBrukerSnartUtlogget = function erBrukerSnartUtlogget(state) {
...
const timeout = (state = initiellState, action = {}) => {
  switch (action.type) {
