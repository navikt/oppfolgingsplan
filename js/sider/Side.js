    ...
    <div className="panel press">
      <div className="hode hode--advarsel">
        ...
        <div className="hode__melding">
          <p>
            Vi jobber med å rette feilen, og håper å være tilbake så snart som
            mulig.
          </p>
        </div>
      </div>
...
export class SideComponent extends Component {
  ...
  render() {
    ...
    const innholdClassNames = cn('side__innhold', {
      'side__innhold--begrenset js-begrensning':
        begrenset || !erInnlogget || !toggleHeleAppen(),
    });
    setAppClass(laster, erInnlogget);

    return (
      <DocumentTitle
        title={tittel + (tittel.length > 0 ? ' - www.nav.no' : 'www.nav.no')}
      >
