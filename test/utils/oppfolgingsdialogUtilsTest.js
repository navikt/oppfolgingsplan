...
describe('OppfolgingdialogUtils', () => {
  ...
  describe('oppgaverOppfoelgingsdialoger', () => {
    ...
    beforeEach(() => {
      sykmeldingsykmeldingUgyldig = hentSykmeldingIkkeGyldigForOppfoelging(
        dagensDato
      );
    ...
    describe('med aktiv sykmelding', () => {
      it('Tom state.oppfoelgingsdialoger.data gir objekt med tomme lister', () => {
        expect(
          oppgaverOppfoelgingsdialoger([], [sykmeldingGyldig])
        ).to.deep.equal({
          nyePlaner: [],
      ...
      it('Finner ny plan', () => {
        ...
        expect(
          oppgaverOppfoelgingsdialoger([dialog], [sykmeldingGyldig])
        ).to.deep.equal({
          nyePlaner: [dialog],
          avventendeGodkjenninger: [],
        });
    ...
    describe('uten aktiv sykmelding', () => {
      it('Tom state.oppfoelgingsdialoger.data gir objekt med tomme lister', () => {
        expect(
          oppgaverOppfoelgingsdialoger([], [sykmeldingsykmeldingUgyldig])
        ).to.deep.equal({
          nyePlaner: [],
          avventendeGodkjenninger: [],
        });
      ...
      it('Finner ikke ny plan', () => {
        ...
        expect(
          oppgaverOppfoelgingsdialoger([dialog], [sykmeldingsykmeldingUgyldig])
        ).to.deep.equal({
          nyePlaner: [],
          avventendeGodkjenninger: [],
        });
  ...
  describe('finnAktiveOppfolgingsdialoger', () => {
    ...
    beforeEach(() => {
      ...
      sykmeldingUtgaatt = {
        ...
        mulighetForArbeid: {
          perioder: [
            {
              fom: leggTilMnderPaaDato(
                dagensDato,
                -(MND_SIDEN_SYKMELDING_GRENSE_FOR_OPPFOELGING + 2)
              ).toISOString(),
              tom: leggTilMnderPaaDato(
                dagensDato,
    ...
    it('finnAktiveOppfolgingsdialoger skal returnere 1 plan, om det eksisterer en plan knyttet til gyldig sykmelding', () => {
      ...
      expect(
        finnAktiveOppfolgingsdialoger(dialog, sykmeldinger)
      ).to.have.length(1);
    });
  ...
  describe('finnBrukersSisteInnlogging', () => {
    it('finnBrukersSisteInnlogging for arbeidsgiver', () => {
      ...
      expect(returnertInnlogging.getTime()).to.equal(
        sisteInnloggingDato.getTime()
      );
    });
  });
