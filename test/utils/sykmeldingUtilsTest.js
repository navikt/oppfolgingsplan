...
export const hentsykmeldingUtgaattOver4mnd = (dagensDato) => {
  return getSykmelding({
    mulighetForArbeid: {
      perioder: [
        {
          fom: trekkMnderFraDato(
            dagensDato,
            MND_SIDEN_SYKMELDING_GRENSE_FOR_OPPFOELGING + 3
          ).toISOString(),
          tom: trekkMnderFraDato(
            dagensDato,
            MND_SIDEN_SYKMELDING_GRENSE_FOR_OPPFOELGING + 2
          ).toISOString(),
        },
        {
          fom: trekkMnderFraDato(
            dagensDato,
            MND_SIDEN_SYKMELDING_GRENSE_FOR_OPPFOELGING + 1
          ).toISOString(),
          tom: trekkMnderOgDagerFraDato(
            dagensDato,
...
describe('sykmeldingUtils', () => {
  ...
  describe('skalViseOppfoelgingsdialogLenke', () => {
    ...
    it('skal returnere true med 1 sykmelding uten orgnummer, med oppfolgingsdialog', () => {
      ...
      expect(
        skalViseOppfoelgingsdialogLenke(sykmeldinger, oppfolgingsdialoger)
      ).to.equal(true);
    });
  ...
  describe('finnArbeidsgivereForGyldigeSykmeldinger', () => {
    it('skal ikke returnere arbeidsgivere, naar sykmelding er utgaatt over 3 maaneder', () => {
      sykmeldinger = [sykmeldingUtgaattOver4mnd];
      expect(
        finnArbeidsgivereForGyldigeSykmeldinger(sykmeldinger, naermesteLedere)
      ).to.have.length(0);
    });
  ...
  describe('finnSykmeldtSinNaermestelederNavnHosArbeidsgiver', () => {
    ...
    it('skal returnerere en naermeste leder', () => {
      ...
      ).to.equal('Navn-Navnolini Navnesen');
    });
  });
  ...
  describe('sykmeldtHarManglendeNaermesteLeder', () => {
    ...
    it('skal returnerere true', () => {
      expect(sykmeldtHarManglendeNaermesteLeder(getArbeidsgivere)).to.equal(
        true
      );
    });
  });
  ...
  describe('sykmeldtHarNaermestelederHosArbeidsgivere', () => {
    it('skal returnerere false', () => {
      ...
      expect(sykmeldtHarNaermestelederHosArbeidsgivere(arbeidsgivere)).to.equal(
        false
      );
    });
    ...
    it('skal returnerere true', () => {
      expect(
        sykmeldtHarNaermestelederHosArbeidsgivere(getArbeidsgivere)
      ).to.equal(true);
    });
  });
});
