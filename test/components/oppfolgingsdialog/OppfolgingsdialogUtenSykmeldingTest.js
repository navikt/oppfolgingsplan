...
describe('OppfolgingsdialogUtenSykmelding', () => {
  ...
  beforeEach(() => {
    ...
    komponent = shallow(
      <OppfolgingsdialogUtenSykmelding oppfolgingsdialog={oppdialoger} />
    );
  });

  it('Viser en div', () => {
    expect(
      komponent.find('div.oppfolgingsdialogUtenAktivSykmelding')
    ).to.have.length(1);
  });

  it('Viser en header', () => {
    expect(
      komponent.find('header.oppfolgingsdialogUtenAktivSykmelding__header')
    ).to.have.length(1);
  });

  it('Viser en div med klass blokk', () => {
    expect(
      komponent.find('div.oppfolgingsdialogUtenAktivSykmelding__blokk')
    ).to.have.length(1);
  });
  ...
  it('Viser en p', () => {
    expect(komponent.find('p.oppfolgingsdialoger__start_tekst')).to.have.length(
      1
    );
  });
});
