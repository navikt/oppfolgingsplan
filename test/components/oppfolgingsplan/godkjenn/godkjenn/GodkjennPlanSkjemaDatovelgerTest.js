...
describe('GodkjennPlanSkjemaDatovelger', () => {
  ...
  beforeEach(() => {
    ...
    komponentDefault = shallow(
      <GodkjennPlanSkjemaDatovelger oppfolgingsplan={oppfolgingsplan} />
    );
  });

  it('Skal vise 2 rader i GodkjennPlanSkjemaDatovelger', () => {
    expect(
      komponentDefault.find('div.godkjennPlanSkjema__datovelger__rad')
    ).to.have.length(2);
  });

  it('Skal vise 3 GodkjennPlanSkjemaDatovelgerFelt', () => {
    expect(
      komponentDefault.find(GodkjennPlanSkjemaDatovelgerFelt)
    ).to.have.length(3);
  });

  it('Skal vise 1 GodkjennPlanSkjemaDatovelgerFelt for startdato', () => {
    expect(
      komponentDefault.find(GodkjennPlanSkjemaDatovelgerFelt).at(0).prop('felt')
        .navn
    ).to.equal(FELTER.fom.navn);
  });

  it('Skal vise 1 GodkjennPlanSkjemaDatovelgerFelt for sluttdato', () => {
    expect(
      komponentDefault.find(GodkjennPlanSkjemaDatovelgerFelt).at(1).prop('felt')
        .navn
    ).to.equal(FELTER.tom.navn);
  });

  it('Skal vise 1 GodkjennPlanSkjemaDatovelgerFelt for evalueringsdato', () => {
    expect(
      komponentDefault.find(GodkjennPlanSkjemaDatovelgerFelt).at(2).prop('felt')
        .navn
    ).to.equal(FELTER.evalueringinnen.navn);
  });
  ...
  describe('GodkjennPlanSkjemaDatovelgerFelt', () => {
    ...
    const komponent = shallow(
      <GodkjennPlanSkjemaDatovelgerFelt felt={felt} dato="01.12.2017" />
    );

    it('Skal vise en GodkjennPlanSkjemaDatovelgerFelt', () => {
      expect(
        komponent.find('div.godkjennPlanSkjema__datovelger__felt')
      ).to.have.length(1);
    });
